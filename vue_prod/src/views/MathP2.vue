<script setup>
    
    import {ref} from 'vue'
    import Answer from '@/components/Response.vue'
    const isOpenMenu = ref(false);
    const toggleMenu = () => {
        isOpenMenu.value = !isOpenMenu.value;
        
    }
</script>

<script>
    import axios from 'axios'
    export default {
        data() {
            return {
                title: 'Default',
                descript: 'Default',
                conditions: 'Default',
                conditionsObj: '',
                answers: []
            }
        },
        async mounted() {
            const response = await axios.get('http://85.192.41.43/API/math/task/1?format=json')
            const result = await response.data.task[1]
            this.title = result.title
            this.descript = result.content
            this.conditions =  result.condition.split('\n')
            for (let i=0; i<this.conditions.length; i++){
                this.conditions[i] = this.conditions[i].replace('+', 'plus').replace('=', 'equal').replace('-', 'minus').split(' ')
            }
        }
    }
    
</script>

<template >
    <h1 class="heading-1">Математика </h1>
    <h2 class="heading-2">{{ title }}</h2>
    <div class="practice">
        <div class="examples">
            <div class="example">
                <img v-bind:src="`../src/img/${this.conditions[0][0]}.svg`" alt="" class="number-visual">
                <img v-bind:src="`../src/img/${this.conditions[0][1]}.svg`" alt="" class="plus">
                <img v-bind:src="`../src/img/${this.conditions[0][2]}.svg`" alt="" class="number-visual">
                <img v-bind:src="`../src/img/${this.conditions[0][3]}.svg`" alt="" class="equal">
                <input v-model="answers[0]" type="text">
            </div>
            <div class="example">
                <img v-bind:src="`../src/img/${this.conditions[1][0]}.svg`" alt="" class="number-visual">
                <img v-bind:src="`../src/img/${this.conditions[1][1]}.svg`" alt="" class="plus">
                <img v-bind:src="`../src/img/${this.conditions[1][2]}.svg`" alt="" class="number-visual">
                <img v-bind:src="`../src/img/${this.conditions[1][3]}.svg`" alt="" class="equal">
                <input type="text" v-model="answers[1]">
            </div>
            <div class="example">
                <img v-bind:src="`../src/img/${this.conditions[2][0]}.svg`" alt="" class="number-visual">
                <img v-bind:src="`../src/img/${this.conditions[2][1]}.svg`" alt="" class="plus">
                <img v-bind:src="`../src/img/${this.conditions[2][2]}.svg`" alt="" class="number-visual">
                <img v-bind:src="`../src/img/${this.conditions[2][3]}.svg`" alt="" class="equal">
                <input type="text" v-model="answers[2]">
            </div>
            <div class="next" @click="toggleMenu"><img src="@/img/next.svg" alt=""></div>
            
        </div>
    </div>
    <span v-if="answers.join('') === [String(conditions[0][4]).replace('\r', ''), String(conditions[1][4]).replace('\r', ''), String(conditions[2][4]).replace('\r', '')].join('')"><Answer :openSidebar="isOpenMenu" v-bind:right="true" :nextStad="'/math/theory3'"/></span>
    <span v-else ><Answer :openSidebar="isOpenMenu" v-bind:right="false"/></span>
</template>

<style lang="scss" scoped>
    input{
        width: 70px;
        outline: none;
        border: none;
        border-bottom: 4px solid #503838;
        transition: 0.3s;
        font-size: 60px;
        color: #000000;
        background-color: #fafafa;
    }

    input:active{
        border-bottom: 4px solid rgba(14, 0, 0, 1);
    }

    .example{
        display: flex;
        align-items: center;
        justify-content: space-between;
        width: 100%;
        & p{
            font-size: 60px;
            color: #000000;
        }
    }
    .plus, .equal{
        width: 50px;
    }

    .number-visual{
        width: 90px;
    }

    @media (max-width: 400px){
        .plus, .equal{
            width: 30px;
        }

        .number-visual{
            width: 60px;
        }
        .example p{
        font-size: 40px;
        }
    }
    .next{
        margin-top: 30px;
        width: 100%;
        height: 70px;
        background: var(--green);
        box-shadow: 0px 5px 10px rgba(24, 46, 79, 0.11);
        border-radius: 15px;
        border: none;
        transition: 0.3s;
        display: flex;
        justify-content: center;
        align-items: center;
    }
</style>