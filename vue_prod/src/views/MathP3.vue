...
<script>
    ...
    export default {
        ...
        async mounted() {
            ...
            const result = await response.data.task[2]
...
<template >
    ...
    <span v-if="answers.join('') === [String(conditions[0][4]).replace('\r', ''), String(conditions[1][4]).replace('\r', ''), String(conditions[2][4]).replace('\r', '')].join('')"><Answer :openSidebar="isOpenMenu" v-bind:right="true" :nextStad="'/math/game'"/></span>
